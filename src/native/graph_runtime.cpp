...
static size_t edge_ring_frame_stride(const EdgeRing &ring) {
    if (ring.frame_stride > 0U) {
        return ring.frame_stride;
    }
    if (!ring.storage) {
        return 0U;
    }
...
static void edge_ring_copy_segment(const EdgeRing &ring, uint32_t start, uint32_t frames, double *buffer, bool to_buffer) {
    if (!ring.storage || ring.capacity == 0U || frames == 0U) {
        return;
    }
...
static uint32_t node_required_block_frames(const AmpGraphRuntime *runtime, const RuntimeNode &node, uint32_t default_frames) {
    ...
    if (frames == 0U) {
        frames = runtime != nullptr && runtime->default_frames > 0U ? runtime->default_frames : 1U;
    }
    if (node.prefill_frames > 0U) {
        frames = std::max(frames, node.prefill_frames);
    }
...
static bool kpn_node_ready(
    ...
    if (!node.output_edge || node.output_edge->capacity == 0U) {
        return false;
    }
    if (edge_ring_free(*node.output_edge) < frames) {
        return false;
    }
    for (const EdgeReader &reader : node.input_edges) {
        ...
        if (edge_ring_available_for_consumer(*reader.ring, reader.consumer_index) < frames) {
            return false;
        }
    }
    return true;
...
static int kpn_execute_node_block(
    ...
    if (runtime == nullptr || node_index >= runtime->nodes.size()) {
        return -1;
    }
    if (frames == 0U) {
        return 0;
    ...
    if (!kpn_node_ready(runtime, node, node_index, frames)) {
    ...
    for (const EdgeReader &reader : node.input_edges) {
        ...
        if (cache.batches > workspace_batches) {
            workspace_batches = cache.batches;
    ...
    size_t output_stride = static_cast<size_t>(workspace_batches) * static_cast<size_t>(out_channels > 0 ? out_channels : 1);
    ...
    std::memcpy(
        node.output->data(),
        frame_buffer,
        static_cast<size_t>(frames) * output_stride * sizeof(double)
    );
    amp_free(frame_buffer);
...
static int runtime_execute_prepass(
    ...
    for (auto &node_ptr : runtime->nodes) {
        if (node_ptr->prefill_frames == 0U) {
            node_ptr->prepass_done = true;
        } else {
            node_ptr->prepass_done = false;
        }
    }
    bool progress = true;
    while (progress) {
        ...
        for (uint32_t idx : runtime->execution_order) {
            ...
            RuntimeNode &node = *runtime->nodes[idx];
            if (node.prefill_frames == 0U || node.prepass_done) {
                continue;
            }
            uint32_t frames = node.prefill_frames;
            if (!kpn_node_ready(runtime, node, idx, frames)) {
                continue;
            }
...
static int kpn_streamer_produce(
    ...
    if (streamer == nullptr || streamer->runtime == nullptr) {
        return -1;
    }
    ...
    if (!sink_edge || sink_edge->capacity == 0U) {
        return -1;
    }
    ...
    if (streamer->batches == 0U) {
        streamer->batches = sink_node.output ? sink_node.output->shape.batches : (runtime->default_batches > 0U ? runtime->default_batches : 1U);
    }
    ...
    if (streamer->frame_stride == 0U) {
        streamer->frame_stride = static_cast<size_t>(streamer->batches) * static_cast<size_t>(std::max<uint32_t>(1U, streamer->channels));
    }
...
static void build_execution_schedule(const AmpGraphRuntime *runtime, std::vector<uint32_t> &out) {
    ...
    while (!ready.empty()) {
        ...
        if (node_idx >= runtime->dependents.size()) {
            continue;
        for (uint32_t dep : runtime->dependents[node_idx]) {
            if (dep >= indegree.size()) {
                continue;
            }
            if (indegree[dep] > 0U) {
                indegree[dep] -= 1U;
    ...
    for (uint32_t order : schedule) {
        ...
        for (size_t view_index = 0; view_index < param_views.size(); ++view_index) {
            if (param_views[view_index].data != nullptr) {
                param_views[view_index].frames = param_tensors[view_index].second->shape.frames;
                param_views[view_index].data = param_tensors[view_index].second->data();
            }
        ...
        if (!used_v2) {
            ...
            return -1;
        auto output_edge = node.output_edge ? node.output_edge : ensure_edge_ring(runtime, node.name, node.vector_policy);
        if (!output_edge) {
            if (frame_buffer != nullptr) {
                amp_free(frame_buffer);
            }
            runtime_set_error(runtime, -1, "edge_ring", &node, "failed to acquire output edge ring");
            return -1;
        if (!node_output) {
            TensorShape shape{};
            shape.batches = batches;
            ...
            if (output_edge->capacity == 0U) {
                output_edge->capacity = node_output->shape.frames;
            }
            uint32_t desired_frames = output_edge->capacity > 0U ? output_edge->capacity : node_output->shape.frames;
            if (node.vector_policy.block_frames > 0U) {
                desired_frames = std::max(desired_frames, node.vector_policy.block_frames);
            }
            desired_frames = align_frames_up(desired_frames, node_output->shape.frames);
            if (desired_frames == 0U) {
                desired_frames = node_output->shape.frames;
            }
            TensorShape ring_shape{};
            ring_shape.batches = node_output->shape.batches;
            ring_shape.channels = node_output->shape.channels;
            ring_shape.frames = desired_frames;
            if (!output_edge->storage
                || node_output->shape.batches != output_edge->storage->shape.batches
                || node_output->shape.channels != output_edge->storage->shape.channels
                || output_edge->storage->shape.frames != ring_shape.frames) {
                output_edge->storage = make_tensor(ring_shape);
                output_edge->storage->shape = ring_shape;
                output_edge->capacity = ring_shape.frames;
                output_edge->head = 0;
                output_edge->tail = 0;
                node.output_ring_head = 0;
            }
            node.output_ring = output_edge->storage;
            node.output_ring_capacity = node.output_ring ? node.output_ring->shape.frames : 0U;
            size_t ring_stride = static_cast<size_t>(node_output->shape.batches) * static_cast<size_t>(node_output->shape.channels);
            if (node.output_ring && node.output_ring_capacity > 0U) {
                if (node.output_ring_head >= node.output_ring_capacity) {
                    node.output_ring_head %= node.output_ring_capacity;
                }
                if (node.output_ring_head + node_output->shape.frames > node.output_ring_capacity) {
                    node.output_ring_head = 0;
                }
                double *ring_base = node.output_ring->data() + static_cast<size_t>(node.output_ring_head) * ring_stride;
                node_output->map_external(
                    ring_base,
                    static_cast<size_t>(node_output->shape.frames) * ring_stride
                );
            } else {
                node_output->clear_external();
            }
        size_t frame_stride = static_cast<size_t>(node_output->shape.batches) * static_cast<size_t>(node_output->shape.channels);
        std::memcpy(
            node_output->data(),
            frame_buffer,
            static_cast<size_t>(frames) * frame_stride * sizeof(double)
        );
        amp_free(frame_buffer);
        state = state_arg;
        ...
        output_edge->head = node.output_ring_head;
        output_edge->capacity = node.output_ring_capacity;
        ...
        if (channel_it != runtime->channels.end()) {
            ...
            channel.block_frames = node.vector_policy.block_frames > 0U
                ? node.vector_policy.block_frames
                : (node_output ? node_output->shape.frames : 0U);
            channel.frame_stride = (channel.block_frames > 0U)
                ? static_cast<size_t>(node_output->shape.batches) * static_cast<size_t>(node_output->shape.channels)
                : 0U;
            if (channel.ring_frames > 0U && channel.block_frames > 0U) {
                uint32_t span = channel.block_frames;
                if (span > channel.ring_frames) {
                    span = channel.ring_frames;
                }
                uint32_t head = channel.ring_frames > 0U ? output_edge->head % channel.ring_frames : 0U;
                uint32_t span_mod = span % channel.ring_frames;
                if (span_mod == 0U) {
                    span_mod = channel.ring_frames;
                }
...
static void streamer_worker_main(AmpGraphStreamer *streamer) {
    ...
    if (streamer->block_frames == 0U) {
        streamer->block_frames = streamer->runtime && streamer->runtime->default_frames > 0U
            ? streamer->runtime->default_frames
            : 64U;
    }
...
AMP_API int amp_graph_runtime_set_scheduler_mode(AmpGraphRuntime *runtime, AmpGraphSchedulerMode mode) {
    ...
    switch (mode) {
        case AMP_SCHEDULER_LEARNED:
            runtime->scheduler_mode = AMP_SCHEDULER_LEARNED;
            break;
        case AMP_SCHEDULER_ORDERED:
        default:
            runtime->scheduler_mode = AMP_SCHEDULER_ORDERED;
            break;
    }
    return 0;
}
...
AMP_API int amp_graph_runtime_set_scheduler_params(AmpGraphRuntime *runtime, const AmpGraphSchedulerParams *params) {
    ...
    if (runtime == nullptr || params == nullptr) {
        return -1;
    }
