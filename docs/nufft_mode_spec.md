...
Status: The FFTDivisionNode forward path now manages a `(lane, cache, bin, time_slice)` working
tensor with optional aggregation policies while the transform core still routes through FFT/DFT
implementations. NUFFT/CZT variants remain in planning and must integrate without bypassing the
cache or introducing Python fallbacks (see docs/spectral_workstation_plan.md).
